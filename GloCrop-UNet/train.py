...
def parse_args():
    ...
    # data
    parser.add_argument('--dataset', default='Wheat', help='dataset name')  ### Wheat, busi, chasedb1, glas
...
"""
config：一个字典，包含配置信息，如是否使用深度监督（deep_supervision）。
train_loader：一个数据加载器，用于从训练集中获取批次数据。
model：要训练的模型。
criterion：损失函数，用于计算模型预测与目标之间的差异。
optimizer：优化器，用于更新模型参数以最小化损失。
函数的主要步骤如下：

初始化平均度量（avg_meters）字典，用于存储损失和IoU（Intersection over Union）的平均值。
将模型设置为训练模式。
创建一个进度条（pbar），用于显示训练进度。
遍历训练数据加载器中的每个批次数据。
将输入和目标数据转移到GPU上。
根据配置计算模型输出和损失。
如果使用深度监督，计算所有输出的损失并求平均值；否则，只计算最后一个输出的损失。
计算IoU和Dice分数。
清零梯度，反向传播损失，并执行优化器步骤以更新模型参数。
更新平均度量字典中的损失和IoU值。
更新进度条的后缀信息。
关闭进度条。
返回包含最终损失和IoU平均值的有序字典。
"""
...
def main():
    ...
    for epoch in range(config['epochs']):
        ...
        pd.DataFrame(log).to_csv('models/%s/log.csv' %
                                 config['name'], index=False,encoding='utf-8')
